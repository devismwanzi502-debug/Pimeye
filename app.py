...
if __name__ == '__main__':
    ...
    app.run(debug=DEBUG, host='0.0.0.0', port=port)        results.append({
            'engine': 'Bing Images',
            'url': bing_search_url,
            'confidence': 85,
            'description': 'Click to search this image on Bing Visual Search — good for visual similarity matching',
...
def search_apify_google_lens(filepath, image_url):
    ...
    try:
        # Upload image to Apify key-value store
        logger.info("Uploading image to Apify key-value store...")
        store = apify_client.key_value_stores().get_or_create(name="osint-image-search")
        kvs = apify_client.key_value_store(store['id'])
        ...
        # Run the actor synchronously and wait for results
        logger.info("Running Google Lens Apify actor — searching all social platforms...")
        run = apify_client.actor("borderline/google-lens").call(run_input={
            "searchTypes": ["exact-match", "visual-match"],
            "imageKvsRecords": [{
        ...
        if items:
            ...
            # Add the full results dataset link as the primary entry
            results.append({
                'engine': 'Google Lens — All Results',
                'url': f"https://console.apify.com/storage/datasets/{run['defaultDatasetId']}",
                'confidence': 98,
            ...
            # Add individual platform results (up to 8)
            platform_count = 0
            for item in items:
                if platform_count >= 8:
                    break
                ...
                if page_url and page_url.startswith('http'):
                    ...
                    # Detect social platform for the icon
                    domain = urllib.parse.urlparse(page_url).netloc.lower()
                    icon = '🌐'
                    if 'instagram' in domain: icon = '📷'
                    elif 'facebook' in domain: icon = '📘'
                    elif 'tiktok' in domain: icon = '🎵'
                    elif 'twitter' in domain or 'x.com' in domain: icon = '🐦'
                    elif 'pinterest' in domain: icon = '📌'
                    elif 'linkedin' in domain: icon = '💼'
                    elif 'reddit' in domain: icon = '👽'
                    elif 'youtube' in domain: icon = '▶️'
                    elif 'tumblr' in domain: icon = '📝'
                    elif 'ebay' in domain or 'amazon' in domain: icon = '🛒'
                    
                    platform_name = domain.replace('www.', '').split('.')[0].capitalize()
                    if platform_name == 'X': platform_name = 'Twitter/X'
            ...
            # If we filtered down, add a note
            if len(items) > platform_count:
                results.append({
                    'engine': f'+ {len(items) - platform_count} more matches',
                    'url': f"https://console.apify.com/storage/datasets/{run['defaultDatasetId']}",
            ...
            # No results but search ran — provide manual link
            results.append({
                'engine': 'Google Lens (All Social Platforms)',
                'url': f"https://lens.google.com/uploadbyurl?url={urllib.parse.quote(image_url)}",
                'confidence': 88,
...
def upload_image():
    ...
    try:
        ...
        # Free search engines (URL-based, no API keys)
        results.extend(search_google_images(filename))
        results.extend(search_tineye_free(filename))
        results.extend(search_bing_images(filename))
        results.extend(search_yandex_images(filename))
        
        # Apify Google Lens — covers all social platforms (Instagram, Facebook, TikTok, Twitter, etc.)
        apify_results = search_apify_google_lens(filepath, image_url)
        results.extend(apify_results)
        
        # Add manual OSINT resources
        results.append({
            'engine': 'FaceCheck.ID',
            'url': 'https://facecheck.id/',
            'confidence': 70,
...
def search_by_url():
    ...
    try:
        ...
        # Get public image URL (the one user provided, or our local URL)
        public_url = get_image_url(filename)
